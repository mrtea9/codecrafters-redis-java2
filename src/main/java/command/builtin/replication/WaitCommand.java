...
public record WaitCommand(int numberOfReplicas, Duration timeout) implements Command {
    ...
    public CommandResponse execute(Redis redis, Client client) {
        ...
        for (final var entry : futures) {
            if (acks.get() >= numberOfReplicas) break;

            final var future = entry.getValue();

            if (remaining <= 0) {
                final var isDone = future.isDone();
                if (isDone) acks.incrementAndGet();
